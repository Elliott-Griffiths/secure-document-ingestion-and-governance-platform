...
1. **GDPR and Data Sovereignty:** Uploaded documents contain personal data. We must ensure data residency (UK regions), prevent unauthorized data exfiltration, enforce data minimisation, and maintain an immutable audit trail.
2. **Online Safety Act & CSEA Content Reporting:** Under the Online Safety (CSEA Content Reporting) Regulations 2026, we have a legal obligation to scan, detect, and preserve illegal content for evidence (1-year retention), without deleting files during active investigations.
3. **Staff Duty of Care:** Safeguarding officers who review flagged content are exposed to potentially graphic, distressing, or illegal material. We must protect their psychological safety and ensure they never download malicious files onto their personal devices.
4. **Secure-by-Default Pipeline:** We must prevent any raw file from reaching permanent storage before validation.

## Decision

We decided to implement a multi-stage gated governance and compliance architecture:

1. **Transient Holding & Quarantine Isolation:** 
...
2. **Staff Wellbeing Controls (Security UI):**
   * The **Security UI** restricts how flagged files are viewed. Files are rendered as low-resolution, greyscale thumbnails hidden behind a mandatory warning message.
...
3. **Data Sovereignty & Security Isolation:**
   * All storage and compute infrastructure is pinned to the UK region (`europe-west2`).
...
* **Operational Ingestion Latency:** Gated validation (ClamAV scans followed by Vision/Video/Document AI checks) introduces ingestion latency, requiring asynchronous queues (Cloud Tasks) for bulk uploads (3+ files).
* **Review Friction:** Security-focused UI safeguards (greyscale/low-res) may occasionally make manual classification more difficult for safeguarding officers, though this is a necessary compromise to protect their wellbeing.