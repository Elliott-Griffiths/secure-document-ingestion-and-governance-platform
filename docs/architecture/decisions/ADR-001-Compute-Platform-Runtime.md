...
I need a compute platform and runtime environment that:
...
### Pros
* **Dynamic Auto-Scaling and Scale-to-Zero:** Cloud Run automatically scales container instances up and down based on traffic. For the Security UI, setting `min-instances = 0` ensures I pay nothing when no security reviews are active.
...
* **Low Footprint and Quick Boot (GoLang):** Using Go ensures fast compilation, lightIight container images, minimal memory usage, and sub-second startup times, which directly reduces the impact of cold starts.
...
### Cons
* **Cold Starts:** Scale-to-zero (`min-instances = 0`) introduces a minor "cold start" latency penalty on initial requests. While acceptable for the internal Security UI, I mitigated this for the citizen-facing API Server by configuring a minimum instance of `min-instances = 1`.
* **Resource and Request Timeout Limits:** Cloud Run enforces a maximum request timeout (60 minutes) and strict CPU/memory limits. To prevent ingestion timeouts during large video uploads, long-running ingestion workflows are offloaded asynchronously to Cloud Tasks rather than handled synchronously within the container's HTTP request lifecycle.
